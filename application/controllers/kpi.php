...
class kpi extends CI_Controller {
    ...
    public function operasional_tahun($tahun) {

        $data['lv1'] = $this->level1;
        $data['lv2'] = 1;
        $data2 = menu_oam();
        $data3['kpi'] = $this->m_kpi->kpi_by_tahun($tahun);
        $data3['depot'] = $this->m_depot->get_depot();
        $data3['tahun_arr'] = array($tahun);
        $data3['tahun'] = $tahun;
        $this->load->view('layouts/header');
        $this->load->view('layouts/menu', $data2);
        $this->navbar($data['lv1'], $data['lv2']);
        $this->load->view("oam/v_kpi_operasional_tahun", $data3);
        $this->load->view('layouts/footer');
    }
